...
| 字段名       | 数据类型    | 约束条件   | 描述               |
| ------------ | ----------- | ---------- | ------------------ |
| address_id   | INT         | 主键、自增 | 地址唯一标识       |
| user_id      | INT         | 非空、外键 | 关联用户表的user_id |
| recipient    | VARCHAR(50) | 非空       | 收件人姓名         |
| phone        | VARCHAR(20) | 非空       | 联系电话           |
| province     | VARCHAR(50) | 非空       | 省份               |
| city         | VARCHAR(50) | 非空       | 城市               |
| district     | VARCHAR(50) | 非空       | 区/县              |
| detail       | VARCHAR(200)| 非空       | 详细地址           |
| is_default   | TINYINT(1)  | 非空, 默认0| 是否默认地址(0:否 1:是) |
...
| address_id   | INT           | 非空、外键 | 地址ID（关联地址表）           |
